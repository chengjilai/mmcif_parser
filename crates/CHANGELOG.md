...
- `BondEdge` is now a `Copy` type reused directly by sparse adjacencies, so both the `mmcif-core` cache and `mmcif-analyze` avoid intermediate cloning when wiring neighbors.
